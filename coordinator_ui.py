...
from PySide6.QtWidgets import (
    ...
    QPushButton, QTableWidget, QTableWidgetItem, QInputDialog, QFileDialog
)
...
class CoordinatorWindow(QWidget):
    def __init__(self):
        ...
        self.deciders_text.setFixedHeight(150)
        ...
        self.matrix_table.cellChanged.connect(self.on_matrix_modified)
    ...
    def upload_matrix(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(self, "Choisir fichier Excel", "", "Excel Files (*.xlsx)", options=options)
        if filename:
            try:
                wb = load_workbook(filename)
                ws = wb.active
                self.matrix = [[str(cell) if cell is not None else "" for cell in row] for row in ws.iter_rows(values_only=True)]
    ...
    def save_matrix(self):
        ...
        self.matrix = [[self.matrix_table.item(i,j).text() if self.matrix_table.item(i,j) else "" for j in range(cols)] for i in range(rows)]
        self.display_matrix_table(self.matrix)

        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getSaveFileName(self, "Enregistrer matrice comme Excel", "", "Excel Files (*.xlsx)", options=options)
        if file_path:
    ...
    def send_matrix(self):
        ...
        try:
            ...
            self.center_text("✅ Matrice envoyée aux décideurs !")
